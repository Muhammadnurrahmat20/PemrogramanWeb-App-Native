...
if ($cmdHapus == "Hapus" && $hdnID != "")
{
    ...
    {
        $notifikasi = "<div class='alert alert-success alert-dismissible fade show' role='alert'>Sukses menghapus sebuah data anggota.<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>";
    }
    else
    {
        $notifikasi = "<div class='alert alert-danger alert-dismissible fade show' role='alert'>Terjadi kesalahan pada sistem. Data tidak dapat dihapus.<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>";
    }
}
                    ...
                    <?php
                        ...
                        while ($data_select_anggota = $kueri_select_anggota->fetch_array())
                        {
                            ...
                            if (isset($_SESSION["NamaPengguna"]))
                            {
                                ...
                                echo "<td><form action='?p=anggota' method='post' onsubmit='return confirm
                                    (`Apakah Anda yakin menghapus data ini?`)'><input type='hidden' name='hdnID' value='".$data_select_anggota["id"]."'><button type='submit' class='btn btn-danger' name='cmdHapus' value='Hapus'>Hapus</button></form></td>";
                            }
                            echo "</tr>";
                            $no++;
                        }
